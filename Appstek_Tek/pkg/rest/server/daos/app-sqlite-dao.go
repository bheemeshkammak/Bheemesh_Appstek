...
import (
	"database/sql"
	"errors"
	"github.com/bheemeshkammak/Bheemesh_Appstek/appstek_tek/pkg/rest/server/daos/clients/sqls"
	"github.com/bheemeshkammak/Bheemesh_Appstek/appstek_tek/pkg/rest/server/models"
...
func (appDao *AppDao) CreateApp(m *models.App) (*models.App, error) {
	insertQuery := "INSERT INTO apps(Employee, App)values(?, ?)"
	res, err := appDao.sqlClient.DB.Exec(insertQuery, m.Employee, m.App)
	if err != nil {
		return nil, err
	}
...
func (appDao *AppDao) UpdateApp(id int64, m *models.App) (*models.App, error) {
	...
	updateQuery := "UPDATE apps SET Employee = ?, App = ? WHERE Id = ?"
	res, err := appDao.sqlClient.DB.Exec(updateQuery, m.Employee, m.App, id)
	if err != nil {
		return nil, err
	}
...
func (appDao *AppDao) ListApps() ([]*models.App, error) {
	...
	for rows.Next() {
		m := models.App{}
		if err = rows.Scan(&m.Id, &m.Employee, &m.App); err != nil {
			return nil, err
		}
		apps = append(apps, &m)
	}
...
func (appDao *AppDao) GetApp(id int64) (*models.App, error) {
	...
	m := models.App{}
	if err := row.Scan(&m.Id, &m.Employee, &m.App); err != nil {
